...
// Add this route to serve the add-anime HTML page
app.get('/add-anime', (req, res) => {
  const filePath = path.join(publicDir, 'add-anime.html');
  res.sendFile(filePath);
});

// Fixed route to add new anime with proper queuing
app.post('/api/add-anime', async (req, res) => {
  try {
    ...
    // Add anime to the END of playlist (queuing it)
    channels[channelId].playlist.push({ 
      title: title.trim(), 
      episodes: sortedEpisodes 
    });
    
    console.log(`📺 [${channelId}] Added anime "${title}" with ${sortedEpisodes.length} episodes to queue`);
    ...
    res.json({ 
      success: true, 
      message: `Added "${title}" to ${channels[channelId].name} queue`,
...
// Fixed route to add episode to existing anime
app.post('/api/add-episode', async (req, res) => {
  try {
    ...
    res.json({ 
      success: true, 
      message: `Added Episode ${episodeNum} to "${animeTitle}"`,
...
// New route to get queue status for a channel
...
// New route to reorder anime in queue
